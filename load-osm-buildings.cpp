 ...
 * load-osm-building
 *
 * code was taken from https://github.com/osmcode/osm-gis-export
 * and modified.
 *
 * Usage:
 * createdb -U postgres -h localhost -p 5435 osm_buildings
 * psql -U postgres -h localhost -p 5435 osm_buildings -c "create extension postgis"
 * psql -U postgres -h localhost -p 5435 osm_buildings -c "create extension hstore"
 * psql -U postgres -h localhost -p 5435 osm_buildings -c "create schema data"
 * load-osm-building -v -f PostgreSQL -o 'PG:host=localhost user=postgres dbname=osm_buildings port=5435' -s data --add-metadata  data/HOLLYWOOD_CA.osm.bz2
 * psql -U postgres -h localhost -p 5435 osm_buildings -c "alter table areas alter column tags type hstore using tags::hstore || ''"
 * psql -U postgres -h localhost -p 5435 osm_buildings -c "select key, count(*) as cnt from (select (each(tags)).* from areas) as foo group by key order by key"
 * psql -U postgres -h localhost -p 5435 osm_buildings -c "select key, count(*) as cnt from (select (each(tags)).* from areas) as foo group by key order by cnt desc"
 *
*/
#include <cstdint>
#include <cstdlib>
...
class MyOGRHandler : public osmium::handler::Handler {
    ...
    static const std::size_t max_length_tags = 2048;
    ...
    void add_metadata_fields(gdalcpp::Layer& layer) {
        ...
        layer.add_field("changeset", OFTReal, 10);
        layer.add_field("timestamp", OFTString, 20);
        layer.add_field("uid", OFTReal, 10);
    ...
    static void add_metadata(gdalcpp::Feature& feature, const osmium::OSMObject& object) {
        ...
        feature.set_field("changeset", double(object.changeset()));
        feature.set_field("timestamp", object.timestamp().to_iso().c_str());
        feature.set_field("uid", double(object.uid()));
...
public:
    ...
    MyOGRHandler(gdalcpp::Dataset& dataset, osmium::geom::OGRFactory<TProjection>& factory, const config& cfg) :
        m_cfg(cfg),
        m_dataset(dataset),
        m_layer_point(dataset, cfg.schema + "points", wkbPoint,
            {"GEOMETRY_NAME=geom","SPATIAL_INDEX=NO","OVERWRITE=YES"}),
        m_layer_linestring(dataset, cfg.schema + "lines", wkbLineString,
            {"GEOMETRY_NAME=geom","SPATIAL_INDEX=NO","OVERWRITE=YES"}),
        m_layer_multipolygon(dataset, cfg.schema + "areas", wkbMultiPolygon,
            {"GEOMETRY_NAME=geom","SPATIAL_INDEX=NO","OVERWRITE=YES"}),
        m_factory(factory) {

        m_layer_point.add_field("id", OFTReal, 10);
        m_layer_linestring.add_field("id", OFTReal, 10);
        m_layer_multipolygon.add_field("id", OFTReal, 10);
    ...
    void node(const osmium::Node& node) {
        const char* building = node.tags()["building"];
        if (building) {
    ...
    void way(const osmium::Way& way) {
        ...
        if (building) {
            try {
                ...
                feature.set_field("id", double(way.id()));
    ...
    void area(const osmium::Area& area) {
        ...
        if (building) {
            try {
                ...
                feature.set_field("id", double(area.id()));
...
void print_help(const po::options_description& desc) {
    std::cout << "load-osm-buildings [OPTIONS] OSM-FILE\n\n"
...
int main(int argc, char* argv[]) {
    ...
    desc.add_options()
        ...
        ("schema,s", po::value<std::string>()->default_value(""), "Schema to create tables in")
    ...
    if (vm.count("schema") and vm["schema"].as<std::string>().size()>0) {
        cfg.schema = vm["schema"].as<std::string>() + ".";
    }
    ...
    gdalcpp::Dataset dataset{output_format, output_filename, gdalcpp::SRS{factory.proj_string()}, { "SPATIALITE=TRUE", "INIT_WITH_EPSG=no", "journal_mode=OFF" }};
